...
import Loading from "./loading"; // Giả sử đây là component Loading bạn đã tạo
...
const ResumeForm = ({ setResult }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    setLoading(true);
    axios
      .post("http://localhost:5000/api/v1/resume/create", formData)
      .then((res) => {
        if (res.data.message) {
          setResult(res.data.data);
          navigate("/resume"); // Dẫn đến trang resume sau khi tạo thành công
        }
      })
      .catch((err) => {
        console.error(err);
  ...
  if (loading) {
    return <Loading />; // Hiển thị Loading nếu đang gửi form
  }
